<template>
  <div class="vchat-myFriend vchat-noUser">
    ...
    <div class="vchat-myFriend-container">
      <div class="me-li" @click="goFriendDetail(user._id)">
        <a href="javascript:;">
          <img :src="photo" alt />
        </a>
        <div>
      ...
      >
        ...
        <ul class="vchat-linkman-list">
          ...
          >
            <a href="javascript:;">
              <img :src="v.photo[0]==='h'?v.photo:IMG_URL + v.photo" alt />
            </a>
            <div>
              <p>
    ...
    <!-- <v-dropdown :command="currFriend" :x="x" :y="y" :visible="visible" @upVisible="upVisible">
      <v-dropdown-item
        ...
        slot="dropdown"
      >{{addOrDel ? '从会话列表移除' : '添加到会话列表'}}</v-dropdown-item>
    </v-dropdown> -->
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    addOrDel() {
      return this.conversationsList.filter(v => v.id === this.currFriend.roomid)
        .length;
    },
    photo() {
      if(this.user.isGitLogin) {
        return this.user.gitPhoto
      } 
      return this.IMG_URL+this.user.photo
    }
  },
  mounted() {
     this.findMyfriends();
     console.log(this.user.gitPhoto)
  }
};
</script>
