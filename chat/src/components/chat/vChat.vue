<template>
  <div
    class="vchat-chatRoom"
    v-bgInmage="IMGURL + user.wallpaper.split(',')[0]"
      ...
      <div class="chat-container">
        <ul class="chat-conversation-ul">
          <li
            ...
            v-for="(v, i) in contactsList"
            :key="v.id"
            :class="{active: currSation.id === v.id}"
            @click="setCurrSation(v)"
          >
            <el-badge :value="v.unRead" :max="99" class="mesBadge" :hidden="v.unRead === 0">
              <a class="vchat-photo">
                <img :src="v.photo[0]==='h'?v.photo:IMGURL + v.photo" alt />
              </a>
            </el-badge>
