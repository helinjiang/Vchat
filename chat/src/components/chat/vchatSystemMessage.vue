    ...
    <ul v-if="InfoList.length">
      <template v-for="v in InfoList">
        <li v-if="v.type === 'validate'" :key="v['_id']">
          <span
            class="vchat-line1 info"
          >{{v.state === 'friend' ? '验证消息：' + v.nickname + '申请加您为好友' : '验证消息：' + v.nickname + '申请加入' + v.groupName}}</span>
          <span class="time">{{v.time}}</span>
          <el-popover placement="left" width="400" v-model="v.visible">
          ...
          <el-popover placement="top" width="160" v-model="v.delVisible">
            <p>确定删除吗？</p>
    ...
    currSation: {
      ...
      handler(v) {
        if (v.id) {
          ...
          this.$store.commit("", { roomid: v.id, clear: true });
          this.$socket.emit("getSystemMessages", {
            roomid: v.id,
            offset: this.offset,
            limit: this.limit
          });
